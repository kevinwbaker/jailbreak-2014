...
class UniversitiesViewTest(CleanTestCase):
    '''This code is a clusterf*ck just well badly test it'''
    fixtures = [
        'teams.json',
        'checkins.json'
    ]
    ...
    def test_200(self):
        '''Tests that when we request Universities that exists we get a 200 response'''
        resp = self.client.get(reverse('universities'))
        self.assertEqual(resp.status_code, 200)

        # test the returned context makes some sense
        self.assertIn('universities', resp.context)
        self.assertEqual(resp.context['best_travellers_distance'], 3190)
        self.assertEqual(resp.context['best_raisers_amount'], 3321)

class UniversityViewTest(CleanTestCase):
    fixtures = [
        'teams.json',
        'checkins.json'
    ]
    ...
    def test_200(self):
        '''Tests that when we request a University that exists we get a 200 response'''
        resp = self.client.get(reverse('university', args=['tcd']))
        self.assertEqual(resp.status_code, 200)

        # test the returned context makes some sense
        self.assertIn('university', resp.context)

    def test_404(self):
        '''Tests that if we lookup a bullshit university we return a 404'''
        resp = self.client.get(reverse('university', args=['nope']))
        self.assertEqual(resp.status_code, 404)
